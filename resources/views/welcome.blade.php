<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="description" content="Falcon video production. Vytvorím Vám svadobný klip, svadobné video, prípadne reklamu. Mesto kde žijem je Banská Bystrica. Na tejto stránke sú moje referencie">

        <link rel="apple-touch-icon" sizes="180x180" href="{{ url('favicon.png') }}">
        <link rel="shortcut icon" href="{{ url('favicon.ico') }}" type="image/x-icon">

        <link href='https://fonts.googleapis.com/css?family=Nunito' rel='stylesheet'>

        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.1/jquery.min.js"></script>

        @vite('resources/css/app.css')

        @vite('resources/js/menu.js')

        <title>Falcon video production</title>
    </head>


    <body class="bg-stone-900 font-nunito">
    <header>
        <div class="min-w-full h-16 ">
            {{--            <h1 class="text-center">Site under construction</h1>--}}
            @include('Layouts.menu')
        </div>
    </header>

    <video class="hidden sm:block  min-h-screen aspect-video" muted autoplay loop>
        <source src="{{ url('Video/Intro.mp4') }}" type="video/mp4">
        <source src="movie.ogg" type="video/ogg">
        Your browser does not support the video tag.
    </video>

    <div class=" flex flex-col items-center">
        <h2 class="text-white text-center text-6xl font-bold pb-10">MOJE SLUŽBY</h2>
        <div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 justify-items-center mb-20">

                <div class="flex flex-row sm:flex-col items-center sm:m-8 py-2 rounded w-72 sm:w-56 h-32 sm:h-52">
                    <div class="border-slate-50 border rounded-full p-4 sm:mb-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="6em" height="6em" viewBox="0 0 32 32"><g fill="white"><path d="M10 4h.5c.28 0 .5.23.5.5c0 .28-.22.5-.5.5H10v2.077c.157.05.309.124.449.223l6.182 4.414a.872.872 0 1 1-1.009 1.418L15 12.69V16h13.296c.487 0 .933.289 1.116.738l.538 1.315a.692.692 0 0 1-.65.947H29v11H4V12.696l-.612.437c-.57.42-1.378 0-1.388-.71c0-.279.14-.549.37-.709L8.55 7.3A1.65 1.65 0 0 1 9 7.077V5h-.5c-.28 0-.5-.22-.5-.5s.22-.5.5-.5H9v-.5c0-.28.22-.5.5-.5s.5.22.5.5V4Zm-.5 14c1.379 0 2.5-1.111 2.5-2.5c0-1.379-1.121-2.5-2.5-2.5A2.502 2.502 0 0 0 7 15.5C7 16.879 8.121 18 9.5 18Zm8.51 9h3v-4.5c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5V27ZM13 29v-3.5c0-1.93-1.57-3.5-3.5-3.5S6 23.57 6 25.5V29h7Zm10.01-6.5V27h3v-4.5c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5Z"/><path d="M26.01 9.79v.04c0 .56-.17 1.09-.46 1.52c-.72 1-1.63 1.87-2.67 2.55c-.11.07-.24.1-.37.1s-.26-.03-.37-.1c-1.04-.68-1.95-1.55-2.67-2.55c-.29-.43-.46-.96-.46-1.52v-.04c.01-1 .81-1.79 1.8-1.79c.14 0 .29.02.42.06c.5.1.92.42 1.16.85c.03.05.07.07.12.07c.05 0 .09-.02.12-.07c.24-.43.66-.75 1.16-.85c.13-.04.28-.06.42-.06c.99 0 1.79.79 1.8 1.79Z"/></g></svg>                    </div>
                    <div>
                        <p class="text-white text-center text-2xl font-bold"> Svadby</p>
                    </div>
                </div>
                <div class="flex flex-row sm:flex-col items-center sm:m-8 py-2 rounded w-72 sm:w-56 h-32 sm:h-52">
                    <div class="border-slate-50 border rounded-full p-4 sm:mb-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="6em" height="6em" viewBox="0 0 512 512"><path fill="white" d="M414.39 97.74A224 224 0 1 0 97.61 414.52A224 224 0 1 0 414.39 97.74ZM64 256.13a191.63 191.63 0 0 1 6.7-50.31c7.34 15.8 18 29.45 25.25 45.66c9.37 20.84 34.53 15.06 45.64 33.32c9.86 16.21-.67 36.71 6.71 53.67c5.36 12.31 18 15 26.72 24c8.91 9.08 8.72 21.52 10.08 33.36a305.36 305.36 0 0 0 7.45 41.27c0 .1 0 .21.08.31C117.8 411.13 64 339.8 64 256.13Zm192 192a193.12 193.12 0 0 1-32-2.68c.11-2.71.16-5.24.43-7c2.43-15.9 10.39-31.45 21.13-43.35c10.61-11.74 25.15-19.68 34.11-33c8.78-13 11.41-30.5 7.79-45.69c-5.33-22.44-35.82-29.93-52.26-42.1c-9.45-7-17.86-17.82-30.27-18.7c-5.72-.4-10.51.83-16.18-.63c-5.2-1.35-9.28-4.15-14.82-3.42c-10.35 1.36-16.88 12.42-28 10.92c-10.55-1.41-21.42-13.76-23.82-23.81c-3.08-12.92 7.14-17.11 18.09-18.26c4.57-.48 9.7-1 14.09.68c5.78 2.14 8.51 7.8 13.7 10.66c9.73 5.34 11.7-3.19 10.21-11.83c-2.23-12.94-4.83-18.21 6.71-27.12c8-6.14 14.84-10.58 13.56-21.61c-.76-6.48-4.31-9.41-1-15.86c2.51-4.91 9.4-9.34 13.89-12.27c11.59-7.56 49.65-7 34.1-28.16c-4.57-6.21-13-17.31-21-18.83c-10-1.89-14.44 9.27-21.41 14.19c-7.2 5.09-21.22 10.87-28.43 3c-9.7-10.59 6.43-14.06 10-21.46c1.65-3.45 0-8.24-2.78-12.75q5.41-2.28 11-4.23a15.6 15.6 0 0 0 8 3c6.69.44 13-3.18 18.84 1.38c6.48 5 11.15 11.32 19.75 12.88c8.32 1.51 17.13-3.34 19.19-11.86c1.25-5.18 0-10.65-1.2-16a190.83 190.83 0 0 1 105 32.21c-2-.76-4.39-.67-7.34.7c-6.07 2.82-14.67 10-15.38 17.12c-.81 8.08 11.11 9.22 16.77 9.22c8.5 0 17.11-3.8 14.37-13.62c-1.19-4.26-2.81-8.69-5.42-11.37a193.27 193.27 0 0 1 18 14.14c-.09.09-.18.17-.27.27c-5.76 6-12.45 10.75-16.39 18.05c-2.78 5.14-5.91 7.58-11.54 8.91c-3.1.73-6.64 1-9.24 3.08c-7.24 5.7-3.12 19.4 3.74 23.51c8.67 5.19 21.53 2.75 28.07-4.66c5.11-5.8 8.12-15.87 17.31-15.86a15.4 15.4 0 0 1 10.82 4.41c3.8 3.94 3.05 7.62 3.86 12.54c1.43 8.74 9.14 4 13.83-.41a192.12 192.12 0 0 1 9.24 18.77c-5.16 7.43-9.26 15.53-21.67 6.87c-7.43-5.19-12-12.72-21.33-15.06c-8.15-2-16.5.08-24.55 1.47c-9.15 1.59-20 2.29-26.94 9.22c-6.71 6.68-10.26 15.62-17.4 22.33c-13.81 13-19.64 27.19-10.7 45.57c8.6 17.67 26.59 27.26 46 26c19.07-1.27 38.88-12.33 38.33 15.38c-.2 9.81 1.85 16.6 4.86 25.71c2.79 8.4 2.6 16.54 3.24 25.21a158 158 0 0 0 4.74 30.07A191.75 191.75 0 0 1 256 448.13Z"/></svg>                    </div>
                    <div class="w-full">
                        <p class="text-white text-center text-2xl font-bold"> Dokumenty</p>
                    </div>
                </div>
                <div class="flex flex-row sm:flex-col items-center sm:m-8 py-2 rounded w-72 sm:w-56 h-32 sm:h-52">
                    <div class="border-slate-50 border rounded-full p-4 sm:mb-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="6em" height="6em" viewBox="0 0 24 24"><path fill="white" d="M18 15h-2v2h2m0-6h-2v2h2m2 6h-8v-2h2v-2h-2v-2h2v-2h-2V9h8M10 7H8V5h2m0 6H8V9h2m0 6H8v-2h2m0 6H8v-2h2M6 7H4V5h2m0 6H4V9h2m0 6H4v-2h2m0 6H4v-2h2m6-10V3H2v18h20V7H12Z"/></svg>                    </div>
                    <div class="w-full">
                        <p class="text-white text-center text-2xl font-bold"> Pre firmy</p>
                    </div>
                </div>
                <div class="flex flex-row sm:flex-col items-center sm:m-8 py-2 rounded w-72 sm:w-56 h-32 sm:h-52">
                    <div class="border-slate-50 border rounded-full p-4 sm:mb-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="6em" height="6em" viewBox="0 0 512 512"><path fill="white" d="M261.938 16.28c-44.12 0-84.047 5.327-113.72 14.282c-14.835 4.478-27.12 9.802-36.374 16.407c-9.238 6.59-16.223 15.3-16.25 26l-.03.03c0 13.315 1.05 26.348 3.03 39c3.29-1.478 6.858-2.368 10.562-2.5c1.437-.05 2.874.004 4.344.188c11.763 1.47 21.494 9.193 28.688 19.187c7.193 9.994 12.08 22.69 13.437 36.375c1.357 13.686-1.013 26.1-7.063 35.594a32.597 32.597 0 0 1-9.937 9.937c30.266 41.24 74.044 67.345 123.313 67.345c48.71 0 92.033-25.536 122.25-65.97c-4.95-2.59-9.168-6.572-12.188-11.31c-6.05-9.495-8.42-21.91-7.063-35.595c1.357-13.686 6.276-26.38 13.47-36.375c7.193-9.994 16.893-17.718 28.656-19.188c.735-.09 1.464-.157 2.187-.187a27.68 27.68 0 0 1 2.156 0c4.872.174 9.506 1.677 13.594 4.094a251.955 251.955 0 0 0 3.28-40.563V73c-.012-10.712-7.002-19.433-16.25-26.03c-9.254-6.606-21.538-11.93-36.374-16.407c-29.672-8.956-69.6-14.282-113.72-14.282zM152.78 48.72c-4.477 3.726-6.936 7.742-6.936 11.936c0 19.348 51.99 35.032 116.094 35.032C326.04 95.688 378 80 378 60.656c0-4.2-2.48-8.206-6.97-11.937c13.2 4.046 23.653 8.826 30.158 13.468c6.634 4.734 8.406 8.483 8.406 10.843c0 2.36-1.772 6.11-8.406 10.845c-6.635 4.734-17.36 9.62-30.938 13.72c-27.155 8.194-65.717 13.468-108.313 13.468c-42.595 0-81.157-5.274-108.312-13.47c-13.577-4.097-24.303-8.984-30.938-13.718c-6.634-4.734-8.406-8.484-8.406-10.844c0-2.36 1.773-6.108 8.407-10.843c6.495-4.634 16.922-9.425 30.094-13.468zm-43.25 79.405a9.547 9.547 0 0 0-1.5.094c-2.917.418-5.155 2.195-7.5 5.874c-3.124 4.904-5.242 13.385-4.218 23.72c.818 8.25 3.468 16.2 7.094 22.686c1.035-8.94 8.625-15.875 17.844-15.875c6.984 0 13.026 3.994 16 9.813c.1-2.316.036-4.772-.22-7.344c-1.023-10.334-4.93-20.226-10.03-27.313c-5.1-7.086-11.195-10.953-15.813-11.53a15.133 15.133 0 0 0-1.656-.125zm301.5 0c-.522.007-1.078.053-1.655.125c-4.618.577-10.71 4.444-15.813 11.53c-5.1 7.088-9.006 16.98-10.03 27.314a51.573 51.573 0 0 0-.25 4.594c3.29-4.295 8.456-7.063 14.28-7.063c9.926 0 17.97 8.043 17.97 17.97c0 .23-.023.457-.032.686c4.477-6.908 7.81-15.998 8.75-25.468c1.024-10.333-1.094-18.814-4.22-23.718c-2.342-3.68-4.58-5.456-7.5-5.875a9.507 9.507 0 0 0-1.5-.095zm-148.905 11.813c11.85 0 22.082 6.965 28.75 16.53c6.668 9.566 10.406 21.998 10.406 35.563c0 13.566-3.737 25.998-10.405 35.564c-6.668 9.565-16.9 16.53-28.75 16.53c-11.85 0-22.05-6.965-28.72-16.53C226.74 218.028 223 205.597 223 192.03c0-13.564 3.74-25.996 10.406-35.56c6.668-9.567 16.868-16.532 28.72-16.532zm0 18.687c-4.606 0-9.29 2.624-13.406 8.53c-4.12 5.908-7.032 14.83-7.032 24.876c0 4.65.64 9.052 1.718 13.032c1.935-7.837 8.97-13.656 17.406-13.656c9.926 0 17.97 8.043 17.97 17.97c0 .62-.033 1.24-.094 1.843c2.38-5.356 3.906-11.975 3.906-19.19c0-10.044-2.945-18.967-7.063-24.874c-4.116-5.907-8.8-8.53-13.405-8.53zm-39.22 128.72a649.773 649.773 0 0 1-4 17.81c.5.183 1.005.357 1.532.533c9.685 3.217 24.14 5.437 40.125 5.437c15.77 0 30.07-2.166 39.75-5.313c-1.495-5.43-2.922-10.995-4.312-16.625c-9.97 2.145-21.07 3.344-32.78 3.344c-14.76 0-28.546-1.884-40.314-5.186zm-8.655 35.967c-11.726 42.656-26.91 77.18-44.656 95.375c.498 1.15 1.65 2.71 4.125 4.72c3.802 3.088 10.298 6.512 18.718 9.406c16.838 5.787 41.21 9.625 68.125 9.625c26.913 0 51.317-3.838 68.156-9.625c8.418-2.894 14.914-6.318 18.717-9.407c3.163-2.568 4.162-4.385 4.407-5.562c-17.226-18.314-33.287-52.22-46.28-94.094c-12.233 3.898-27.832 6.063-45.002 6.063c-17.664 0-33.65-2.294-46.03-6.407c-.096-.03-.187-.062-.282-.094zm152.563 106.25c-2 3.152-4.644 5.948-7.594 8.344c-6.39 5.19-14.64 9.226-24.44 12.594c-19.6 6.737-45.566 10.625-74.217 10.625c-28.652 0-54.588-3.888-74.188-10.625c-9.8-3.368-18.05-7.405-24.438-12.594c-2.89-2.348-5.484-5.082-7.468-8.156c-.4.18-.788.398-1.19.563c-11.745 4.817-21.073 11.418-23.468 21.53c-4.48 18.93 58.848 38.31 131.875 38.313c73.028.002 132.407-17.273 132.407-38.312c0-8.636-11.4-13.37-27.28-22.28z"/></svg>                    </div>
                    <div class="w-full">
                        <p class="text-white text-center text-2xl font-bold"> Prijímania</p>
                    </div>
                </div>
                <div class="flex flex-row sm:flex-col items-center sm:m-8 py-2 rounded w-72 sm:w-56 h-32 sm:h-52">
                    <div class="border-slate-50 border rounded-full p-4 sm:mb-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="6em" height="6em" viewBox="0 0 20 20"><path fill="white" d="M15.987 3.016a1 1 0 0 0-1.285-.79l-7 2.187A1 1 0 0 0 7 5.368V13.5a2.5 2.5 0 1 0 1 2V8.368l7-2.188v2.843c.343.03.678.093 1 .184V3.18c0-.056-.005-.11-.013-.164ZM19 14.5a4.5 4.5 0 1 1-9 0a4.5 4.5 0 0 1 9 0Zm-2.263-.435l-2.994-1.65a.5.5 0 0 0-.742.437v3.3a.5.5 0 0 0 .742.438l2.994-1.65a.5.5 0 0 0 0-.875Z"/></svg>                    </div>
                    <div class="w-full">
                        <p class="text-white text-center text-2xl font-bold"> Klipy</p>
                    </div>
                </div>
            </div>
        </div>
    </div>


        <footer>
            <div class="w-full max-h-20">
                {{--            <h1 class="text-center">Site under construction</h1>--}}
                @include('Layouts.footer')
            </div>
        </footer>
    </body>


</html>
